...
uint8_t aes_mult2(uint8_t b) {
	if ((b & 0x80u) != 0u) {
		return (uint8_t)(((b << 1) & 0xFFu) ^ 0x1Bu);
	}
	return (uint8_t)((b << 1) & 0xFFu);
}

uint8_t aes_mult(uint8_t a, uint8_t m) {
	uint8_t product = 0x00u;
	uint8_t current = a;

	for (int i = 0; i < 8; i++) {
		if (((m >> i) & 0x1u) != 0u) {
			product = aes_add(product, current);
		}
		current = aes_mult2(current);
	}
...
void mix_columns_state(uint8_t state[4][4]) {
	...
	for (int row = 0; row < 4; row++) {
		for (int col = 0; col < 4; col++) {
			uint8_t acc = 0x00u;
			for (int k = 0; k < 4; k++) {
				acc = aes_add(acc, aes_mult(state[k][col], M[row][k]));
			}
			out[row][col] = acc;
		}
	}
...
void add_round_key_state(uint8_t state[4][4], const uint8_t round_key[4][4]) {
	for (int row = 0; row < 4; row++) {
		for (int col = 0; col < 4; col++) {
			state[row][col] = aes_add(state[row][col], round_key[row][col]);
		}
	}
}
