...
def read_fpga_summaries(synth_folder: Path, sim_performance):
    points = []

    for summary_file in sorted(synth_folder.glob("*/*/FPGA/reports/summary.csv")):
        ...
        try:
            lut = float(row["lut"])
            lut_logic = float(row.get("lut_logic", lut))
            lut_memory = float(row.get("lut_memory", 0.0))
            lut_shift = float(row.get("lut_shift", 0.0))
            lut_total = lut_logic + lut_memory + lut_shift
            ff = float(row["ff"])
            dsp = float(row["dsp"])
            bram = float(row["bram"])
...
def plot_pareto(points, output_file: Path):
    ...
    ax.set_title("FPGA Pareto: Area vs Throughput")
    ax.set_xlabel("Area score (LUT + FF + DSP + BRAM)")
...
def main():
    ...
    output_file = Path(
        os.environ.get("PLOT_SYNTH_FPGA_PARETO", synth_folder / "plots" / "pareto_fpga.png")
    )
    ...
    points = read_fpga_summaries(synth_folder, sim_performance)
    if not points:
        print(
            f"ERROR: no matching FPGA summary.csv and performance.csv files found in {synth_folder}",
            file=sys.stderr,
        )
        return 1

    plot_pareto(points, output_file)
    print(f"Saved FPGA Pareto plot: {display_path(output_file, project_root)}")
