...
if [[ -f "$PRJ_FILE_SYNTH_FPGA" ]] && grep -q "$KECCAK_RTL_SERVER_REL/" "$PRJ_FILE_SYNTH_FPGA"; then
...
server_shareFolder_PCtoServer "$SYNTH_FPGA_SCRIPTS_FOLDER"
...
set +e
server_executeScript "$SCRIPT_RUN_SYNTH_FPGA_SERVER" "${SERVER_SYNTH_ARGS[@]}"
synth_status=$?
set -e

# Retrive only the current module FPGA synthesis results from the server.
FPGA_RESULT_REL=$(general_stripProjectRoot "$SYNTH_FPGA_RESULT_FOLDER")
print_phase "Receive Results From Server"
echo "Results: $FPGA_RESULT_REL"
server_shareFolder_ServerToPC "$SYNTH_FPGA_RESULT_FOLDER"
