...
module data_sink #(
	...
	input	logic			valid_o,
	input	logic			busy_o,
	input	logic [REQ_NUM_BITS-1:0]	output_RN_o
);
	...
	// 1) Track accepted requests: request_RN_i && ready_o.
	// 2) Compare output_RN_o against Golden Model only when valid_o is high.
	...
	always @(posedge clk_i) begin
		...
		logic [REQ_NUM_BITS-1:0] output_RN_o_gold;
		int scan_status;
		bit request_accepted;

		cycle_errs = 0;
		request_accepted = (request_RN_i && ready_o);
		...
		// Compare output_RN_o only when valid_o is asserted
		if (valid_o) begin
			...
			end else if (output_RN_o != output_RN_o_gold) begin
				cycle_errs++;
				$display("[TB][ERR][cycle %0d] Mismatch output_RN_o at valid index %0d.", cycle_idx, valid_idx);
				$display("\t-> GoldModel = 0x%0h", output_RN_o_gold);
				$display("\t-> HDL       = 0x%0h", output_RN_o);
			end
	...
	final begin
		logic [REQ_NUM_BITS-1:0] trailing_gold_value;
